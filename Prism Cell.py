...
class Game():
    ...
    def __init__(self):
        ...
        self.lines = []
        self.click = False
        self.reflect_sound = pygame.mixer.Sound('mixkit-sci-fi-positive-notification-266.wav')
        self.reflect_sound.set_volume(0.1)
        self.refract_sound = pygame.mixer.Sound('mixkit-sci-fi-interface-zoom-890.wav')
        self.refract_sound.set_volume(0.1)
        self.detect_sound = pygame.mixer.Sound('mixkit-sci-fi-click-900.wav')
        self.detect_sound.set_volume(0.25)
        self.split_sound = pygame.mixer.Sound('mixkit-futuristic-robotic-fast-sweep-171.wav')
...
def main():
    ...
    windowSurface.fill(BLACK)
    count = 0
    pygame.mixer.music.load('quads.mp3')
    ...
    basicFont = pygame.font.Font('BrickshaperseXPx.ttf', 56)
    text = basicFont.render('You took ' + str(round(time.time() - start_time, 2)) + ' to complete the game!', True, WHITE)
    textrect = text.get_rect()
